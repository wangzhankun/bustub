...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (args.size() != 1) {
    throw Exception("args size is not 1");
  }

  AbstractExpressionRef ret;
  if (func_name == "lower") {
    ret = std::make_shared<StringExpression>(std::move(args[0]), StringExpressionType::Lower);
  } else if (func_name == "upper") {
    ret = std::make_shared<StringExpression>(std::move(args[0]), StringExpressionType::Upper);
  } else {
